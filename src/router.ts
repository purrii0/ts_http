...
import { getContentType, resolveSafePath } from "./utils.js";
import { requestParser } from "./parser.js";
import { buildResponse } from "./response.js";
import { requestLogger } from "./logger.js";
import { getCachedOrRead } from "./cache.js";
...
export async function handleRouteAsync(rawRequest: string, socket: Socket) {
  ...
  if (stat.isDirectory()) {
    ...
    const stats = await Promise.all(
      files.map(async (f) => ({
        f,
        stat: await fsp.stat(path.join(resolved, f)),
      })),
    );
  ...
  socket.end(cached.content, () =>
    requestLogger(socket, req, 200, cached.size, startTime),
  );
}
